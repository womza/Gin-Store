...
include_once(_PS_MODULE_DIR_.'dreamslider/DreamSlide.php');

class DreamSlider extends Module
{
	private $_html = '';

	public function __construct()
	{
		$this->name = 'dreamslider';
		...
		$this->displayName = $this->l('DreamSlider');
		$this->description = $this->l('Adds Slideshow images to your homepage.');
	}
	...
	public function install()
	{
		...
		if (parent::install() && $this->registerHook('displayHome') && $this->registerHook('actionShopDataDuplication'))
		{
			...
			$res = Configuration::updateValue('DREAMSLIDER_WIDTH', '556');
			$res &= Configuration::updateValue('DREAMSLIDER_HEIGHT', '325');
			$res &= Configuration::updateValue('DREAMSLIDER_SPEED', '500');
			$res &= Configuration::updateValue('DREAMSLIDER_PAUSE', '3000');
			$res &= Configuration::updateValue('DREAMSLIDER_LOOP', '1');
			/* Creates tables */
	...
	public function uninstall()
	{
		...
		if (parent::uninstall())
		{
			...
			$res &= Configuration::deleteByName('DREAMSLIDER_WIDTH');
			$res &= Configuration::deleteByName('DREAMSLIDER_HEIGHT');
			$res &= Configuration::deleteByName('DREAMSLIDER_SPEED');
			$res &= Configuration::deleteByName('DREAMSLIDER_PAUSE');
			$res &= Configuration::deleteByName('DREAMSLIDER_LOOP');
			return $res;
		}
		return false;
	}
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'dreamslider` (
				`id_dreamslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_dreamslider_slides`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'dreamslider_slides` (
			  `id_dreamslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `position` int(10) unsigned NOT NULL DEFAULT \'0\',
			  `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
			  PRIMARY KEY (`id_dreamslider_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides lang configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'dreamslider_slides_lang` (
			  `id_dreamslider_slides` int(10) unsigned NOT NULL,
			  ...
			  PRIMARY KEY (`id_dreamslider_slides`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new DreamSlide($slide['id_slide']);
			$to_del->delete();
		}
		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'dreamslider`, `'._DB_PREFIX_.'dreamslider_slides`, `'._DB_PREFIX_.'dreamslider_slides_lang`;
		');
	}
	...
	private function _displayForm()
	{
		...
		$this->_html .= '
			...
			<div class="margin-form">
				<input type="text" name="DREAMSLIDER_HEIGHT" id="speed" size="3" value="'.Configuration::get('DREAMSLIDER_HEIGHT').'" /> px
			</div>';
		/* Width field */
		$this->_html .= '
		<label>'.$this->l('Width:').'</label>
		<div class="margin-form">
			<input type="text" name="DREAMSLIDER_WIDTH" id="pause" size="3" value="'.Configuration::get('DREAMSLIDER_WIDTH').'" /> px
		</div>';
		/* Speed field */
		$this->_html .= '
			<label>'.$this->l('Speed:').'</label>
			<div class="margin-form">
				<input type="text" name="DREAMSLIDER_SPEED" id="speed" size="3" value="'.Configuration::get('DREAMSLIDER_SPEED').'" /> ms
			</div>';
		/* Pause field */
		$this->_html .= '
		<label>'.$this->l('Pause:').'</label>
		<div class="margin-form">
			<input type="text" name="DREAMSLIDER_PAUSE" id="pause" size="3" value="'.Configuration::get('DREAMSLIDER_PAUSE').'" /> ms
		</div>';
		/* Loop field */
		$this->_html .= '
		...
		<div class="margin-form">
			...
			<input type="radio" name="DREAMSLIDER_LOOP" id="loop_on" '.(Configuration::get('DREAMSLIDER_LOOP') == 1 ? 'checked="checked"' : '').' value="1" />
			<label class="t" for="loop_on">'.$this->l('Yes').'</label>
			<img src="../img/admin/disabled.gif" alt="No" title="No" style="margin-left: 10px;" />
			<input type="radio" name="DREAMSLIDER_LOOP" id="loop_off" '.(Configuration::get('DREAMSLIDER_LOOP') == 0 ? 'checked="checked" ' : '').' value="0" />
	...
	private function _displayAddForm()
	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slide = new DreamSlide((int)Tools::getValue('id_slide'));
		...
		foreach ($languages as $language)
		{
			...
			/* Display image */
			if ($slide && $slide->image[$language['id_lang']])
				$this->_html .= '<input type="hidden" name="has_picture" value="1" /><img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/img/images/'.$slide->image[$language['id_lang']].'" width="'.(Configuration::get('DREAMSLIDER_WIDTH')/2).'" height="'.(Configuration::get('DREAMSLIDER_HEIGHT')/2).'" alt="" />';
			$this->_html .= '</div>';
		}
	...
	private function _postValidation()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			...
			if (!Validate::isInt(Tools::getValue('DREAMSLIDER_SPEED')) || !Validate::isInt(Tools::getValue('DREAMSLIDER_PAUSE')) ||
				!Validate::isInt(Tools::getValue('DREAMSLIDER_WIDTH')) || !Validate::isInt(Tools::getValue('DREAMSLIDER_HEIGHT')))
		...
		elseif (Tools::isSubmit('submitSlide'))
		{
			...
			foreach ($languages as $language)
			{
				...
				if (Tools::strlen(Tools::getValue('legend_'.$language['id_lang'])) > 40)
					$errors[] = $this->l('Legend is too long');
				if (Tools::strlen(Tools::getValue('url_'.$language['id_lang'])) > 200)
					$errors[] = $this->l('URL is too long');
				if (Tools::strlen(Tools::getValue('description_'.$language['id_lang'])) > 400)
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			$res = Configuration::updateValue('DREAMSLIDER_WIDTH', (int)Tools::getValue('DREAMSLIDER_WIDTH'));
			$res &= Configuration::updateValue('DREAMSLIDER_HEIGHT', (int)Tools::getValue('DREAMSLIDER_HEIGHT'));
			$res &= Configuration::updateValue('DREAMSLIDER_SPEED', (int)Tools::getValue('DREAMSLIDER_SPEED'));
			$res &= Configuration::updateValue('DREAMSLIDER_PAUSE', (int)Tools::getValue('DREAMSLIDER_PAUSE'));
			$res &= Configuration::updateValue('DREAMSLIDER_LOOP', (int)Tools::getValue('DREAMSLIDER_LOOP'));
			if (!$res)
			...
			foreach ($languages as $language)
			{
					...
					in_array($type, array('jpg', 'gif', 'jpeg', 'png')))
				{
					...
					elseif (!$temp_name || !move_uploaded_file($_FILES['image_'.$language['id_lang']]['tmp_name'], $temp_name))
						return false;
					elseif (!ImageManager::resize($temp_name, dirname(__FILE__).'/img/images/'.Tools::encrypt($_FILES['image_'.$language['id_lang']]['name'].$salt).'.'.$type))
		...
		{
			$slide = new DreamSlide((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			if (!$res)
	...
	{
		$slider = array(
			'width' => Configuration::get('DREAMSLIDER_WIDTH'),
			'height' => Configuration::get('DREAMSLIDER_HEIGHT'),
			'speed' => Configuration::get('DREAMSLIDER_SPEED'),
			'pause' => Configuration::get('DREAMSLIDER_PAUSE'),
			'loop' => Configuration::get('DREAMSLIDER_LOOP'),
		);
		...
		$this->smarty->assign('dreamslider_slides', $slides);
		$this->smarty->assign('dreamslider', $slider);
		return true;
	}
	...
	public function hookDisplayHome()
	{
		...
		$this->context->controller->addJS($this->_path.'js/jquery.dmSlider.min.js');
		$this->context->controller->addCSS($this->_path.'css/dm_styles.css');
		$this->context->controller->addJS($this->_path.'js/dreamslider.js');
		return $this->display(__FILE__, 'dreamslider.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT INTO '._DB_PREFIX_.'dreamslider (id_dreamslider_slides, id_shop)
			SELECT id_dreamslider_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'dreamslider
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				...
				FROM `'._DB_PREFIX_.'dreamslider_slides` hss, `'._DB_PREFIX_.'dreamslider` hs
				WHERE hss.`id_dreamslider_slides` = hs.`id_dreamslider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_dreamslider_slides` as id_slide,
					   hssl.`image`,
					   hss.`position`,
					   hss.`active`,
					   hssl.`title`,
					   ...
					   hssl.`legend`,
					   hssl.`description`
			FROM '._DB_PREFIX_.'dreamslider hs
			LEFT JOIN '._DB_PREFIX_.'dreamslider_slides hss ON (hs.id_dreamslider_slides = hss.id_dreamslider_slides)
			LEFT JOIN '._DB_PREFIX_.'dreamslider_slides_lang hssl ON (hss.id_dreamslider_slides = hssl.id_dreamslider_slides)
	...
	{
		$req = 'SELECT hs.`id_dreamslider_slides` as id_slide
				FROM `'._DB_PREFIX_.'dreamslider` hs
				WHERE hs.`id_dreamslider_slides` = '.(int)$id_slide;
