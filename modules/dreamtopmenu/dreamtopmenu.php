...
require (dirname(__FILE__).'/dreamlinks.class.php');

class Dreamtopmenu extends Module
{
	private $_menu = '';
	private $_html = '';
	private $user_groups;
	...
	{
		$this->name = 'dreamtopmenu';
		$this->tab = 'front_office_features';
		$this->version = 1.5;
		$this->author = 'Dreamtheme';

		parent::__construct();

		$this->displayName = $this->l('Dreamtheme horizontal menu');
	...
	public function install()
	{
		if (!parent::install() ||
			...
			!Configuration::updateGlobalValue('MOD_DREAMTOPMENU_ITEMS', 'CAT2,LNK2,LNK3,LNK4,LNK5,LNK6') ||
			!Configuration::updateGlobalValue('MOD_DREAMTOPMENU_SEARCH', '1') ||
	...
	public function installDb()
	{
		...
		CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'dreammenutop` (
			`id_dreammenutop` INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`new_window` TINYINT( 1 ) NOT NULL,
			INDEX (`id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
			Db::getInstance()->execute('
			 CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'dreammenutop_lang` (
			`id_dreammenutop` INT(11) UNSIGNED NOT NULL,
			`id_lang` INT(11) UNSIGNED NOT NULL,
			`id_shop` INT(11) UNSIGNED NOT NULL,
			`label` VARCHAR( 128 ) NOT NULL ,
			`link` VARCHAR( 128 ) NOT NULL ,
			INDEX ( `id_dreammenutop` , `id_lang`, `id_shop`)
		) ENGINE = '._MYSQL_ENGINE_.' CHARACTER SET utf8 COLLATE utf8_general_ci;') &&
		...
		DbCore::getInstance()->Execute(
      "INSERT INTO `" . _DB_PREFIX_ . "dreammenutop` (`id_dreammenutop`, `id_shop`, `new_window`) VALUES(1, 1, 0);"
    ) &&
			DbCore::getInstance()->Execute(
      "INSERT INTO `" . _DB_PREFIX_ . "dreammenutop` (`id_dreammenutop`, `id_shop`, `new_window`) VALUES(2, 1, 0);"
    ) &&
			DbCore::getInstance()->Execute(
      "INSERT INTO `" . _DB_PREFIX_ . "dreammenutop` (`id_dreammenutop`, `id_shop`, `new_window`) VALUES(3, 1, 0);"
    ) &&
			DbCore::getInstance()->Execute(
      "INSERT INTO `" . _DB_PREFIX_ . "dreammenutop` (`id_dreammenutop`, `id_shop`, `new_window`) VALUES(4, 1, 0);"
    ) &&
			DbCore::getInstance()->Execute(
      "INSERT INTO `" . _DB_PREFIX_ . "dreammenutop` (`id_dreammenutop`, `id_shop`, `new_window`) VALUES(5, 1, 0);"
    ) &&
			DbCore::getInstance()->Execute(
      "INSERT INTO `" . _DB_PREFIX_ . "dreammenutop` (`id_dreammenutop`, `id_shop`, `new_window`) VALUES(6, 1, 0);"
    ) &&
	...
	DbCore::getInstance()->Execute(
	"INSERT INTO `" . _DB_PREFIX_ . "dreammenutop_lang` (`id_dreammenutop`, `id_lang`, `id_shop`, `label`, `link`) VALUES(1, 1, 1, 'Home', 'index.php');"
    )&&
		DbCore::getInstance()->Execute(
	"INSERT INTO `" . _DB_PREFIX_ . "dreammenutop_lang` (`id_dreammenutop`, `id_lang`, `id_shop`, `label`, `link`) VALUES(2, 1, 1, 'New Products', 'index.php?controller=new-products');"
    )&&
		DbCore::getInstance()->Execute(
	"INSERT INTO `" . _DB_PREFIX_ . "dreammenutop_lang` (`id_dreammenutop`, `id_lang`, `id_shop`, `label`, `link`) VALUES(3, 1, 1, 'Specials', 'index.php?controller=best-sales');"
    )&&
		DbCore::getInstance()->Execute(
	"INSERT INTO `" . _DB_PREFIX_ . "dreammenutop_lang` (`id_dreammenutop`, `id_lang`, `id_shop`, `label`, `link`) VALUES(4, 1, 1, 'My Account', 'index.php?controller=my-account');"
    )&&
		DbCore::getInstance()->Execute(
	"INSERT INTO `" . _DB_PREFIX_ . "dreammenutop_lang` (`id_dreammenutop`, `id_lang`, `id_shop`, `label`, `link`) VALUES(5, 1, 1, 'Sitemap', 'index.php?controller=sitemap');"
    )&&
		DbCore::getInstance()->Execute(
	"INSERT INTO `" . _DB_PREFIX_ . "dreammenutop_lang` (`id_dreammenutop`, `id_lang`, `id_shop`, `label`, `link`) VALUES(6, 1, 1, 'Contacts', 'index.php?controller=contact');"
	));
	...
	{
		if (!parent::uninstall() ||
			!Configuration::deleteByName('MOD_DREAMTOPMENU_ITEMS') ||
			!Configuration::deleteByName('MOD_DREAMTOPMENU_SEARCH') ||
			!$this->uninstallDB())
			return false;
		return true;
	}
	...
	{
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'dreammenutop`');
		Db::getInstance()->execute('DROP TABLE `'._DB_PREFIX_.'dreammenutop_lang`');
		return true;
	}
	...
	public function getContent()
	{
		...
		if (Tools::isSubmit('submitDreamtopmenu'))
		{
			if (Configuration::updateValue('MOD_DREAMTOPMENU_ITEMS', Tools::getValue('items')))
				$this->_html .= $this->displayConfirmation($this->l('Settings Updated'));
			else
				$this->_html .= $this->displayError($this->l('Unable to update settings'));
			Configuration::updateValue('MOD_DREAMTOPMENU_SEARCH', (bool)Tools::getValue('search'));
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitDreamtopmenuLinks'))
		{
			...
			{
				dreamlinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)Shop::getContextShopID());
				$this->_html .= $this->displayConfirmation($this->l('The link has been added'));
			}
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitDreamtopmenuRemove'))
		{
			$id_dreammenutop = Tools::getValue('id_dreammenutop', 0);
			dreamlinks::remove($id_dreammenutop, (int)Shop::getContextShopID());
			Configuration::updateValue('MOD_DREAMTOPMENU_ITEMS', str_replace(array('LNK'.$id_dreammenutop.',', 'LNK'.$id_dreammenutop), '', Configuration::get('MOD_DREAMTOPMENU_ITEMS')));
			$this->_html .= $this->displayConfirmation($this->l('The link has been removed'));
			$update_cache = true;
		}
		else if (Tools::isSubmit('submitDreamtopmenuEdit'))
		{
			$id_dreammenutop = (int)Tools::getValue('id_dreammenutop', 0);
			$id_shop = (int)Shop::getContextShopID();

			if (!Tools::isSubmit('link'))
			{
				$tmp = dreamlinks::getLinkLang($id_dreammenutop, $id_shop);
			...
			{
				dreamlinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int)$id_shop, (int)$id_dreammenutop, (int)$id_dreammenutop);
		...
		<fieldset>
			...
			<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post" id="form">
				...
				<div class="margin-form">
					<input type="text" name="items" id="itemsInput" value="'.Tools::safeOutput(Configuration::get('MOD_DREAMTOPMENU_ITEMS')).'" size="70" />
				</div>
				</div>
		...
		$links = dreamlinks::gets($id_lang, null, (int)Shop::getContextShopID());
		foreach ($links as $link)
		{
			if ($link['label'] == '')
			{
				$link = dreamlinks::get($link['id_dreammenutop'], $default_language, (int)Shop::getContextShopID());
				$this->_html .= '<option value="LNK'.(int)$link[0]['id_dreammenutop'].'">'.$spacer.$link[0]['label'].'</option>';
			}
			else
				$this->_html .= '<option value="LNK'.(int)$link['id_dreammenutop'].'">'.$spacer.$link['label'].'</option>';
		}
		$this->_html .= '</optgroup>';
		...
		$this->_html .= '</select><br/>
				...
				<div class="margin-form">
					<input type="checkbox" name="search" id="s" value="1"'.((Configuration::get('MOD_DREAMTOPMENU_SEARCH')) ? ' checked=""': '').'/>
				</div>
				<p class="center">
					<input type="submit" name="submitDreamtopmenu" value="'.$this->l('	Save	').'" class="button" />
				</p>
			</form>
		</fieldset><br />';
...
<div class="margin-form">';
		...
		if (Tools::isSubmit('id_dreammenutop'))
			$this->_html .= '<input type="hidden" name="id_dreammenutop" value="'.(int)Tools::getValue('id_dreammenutop').'" />';

		if (Tools::isSubmit('submitDreamtopmenuEdit'))
			$this->_html .= '<input type="submit" name="submitDreamtopmenuEdit" value="'.$this->l('Edit').'" class="button" />';

		$this->_html .= '
					<input type="submit" name="submitDreamtopmenuLinks" value="'.$this->l('	Add	').'" class="button" />
</div>

			</form>
		</fieldset><br />';

		$links = dreamlinks::gets((int)$id_lang, null, (int)Shop::getContextShopID());
		...
		foreach ($links as $link)
		{
			$this->_html .= '
					<tr>
						<td>'.(int)$link['id_dreammenutop'].'</td>
						...
						<td>
							<form action="'.Tools::safeOutput($_SERVER['REQUEST_URI']).'" method="post">
								<input type="hidden" name="id_dreammenutop" value="'.(int)$link['id_dreammenutop'].'" />
								<input type="submit" name="submitDreamtopmenuEdit" value="'.$this->l('Edit').'" class="button" />
								<input type="submit" name="submitDreamtopmenuRemove" value="'.$this->l('Remove').'" class="button" />
							</form>
						</td>
					</tr>';
		}
	...
	{
		return explode(',', Configuration::get('MOD_DREAMTOPMENU_ITEMS'));
	}
	...
	private function makeMenuOption()
	{
		...
		foreach ($menu_item as $item)
		{
			...
			switch (substr($item, 0, strlen($values[1])))
			{
				...
				case 'LNK':
					$link = dreamlinks::get((int)$id, (int)$id_lang, (int)$id_shop);
					if (count($link))
					{
						if (!isset($link[0]['label']) || ($link[0]['label'] == ''))
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = dreamlinks::get($link[0]['id_dreammenutop'], (int)$default_language, (int)Shop::getContextShopID());
						}
						$this->_html .= '<option value="LNK'.$link[0]['id_dreammenutop'].'">'.$link[0]['label'].'</option>';
					}
					break;
				case 'SHOP':
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'LNK':
					...
					if (count($link))
					{
						...
						{
							$default_language = Configuration::get('PS_LANG_DEFAULT');
							$link = dreamlinks::get($link[0]['id_dreammenutop'], $default_language, (int)Shop::getContextShopID());
						}
	...
	public function hookDisplayTop($param)
	{
		...
		if (!$this->isCached('dreamtopmenu.tpl', $this->getCacheId()))
		{
			$this->makeMenu();
			$this->smarty->assign('MENU_SEARCH', Configuration::get('MOD_DREAMTOPMENU_SEARCH'));
		...
		$html = $this->display(__FILE__, 'dreamtopmenu.tpl', $this->getCacheId());
		return $html;
	}
	...
	{
		$this->_clearCache('dreamtopmenu.tpl');
	}
	public function hookActionShopDataDuplication($params)
	{
		$dreammenutop = Db::getInstance()->executeS('
			SELECT *
			FROM '._DB_PREFIX_.'dreammenutop 
			WHERE id_shop = '.(int)$params['old_id_shop']
			);

		foreach($dreammenutop as $id => $link)
		{
			Db::getInstance()->execute('
				INSERT IGNORE INTO '._DB_PREFIX_.'dreammenutop (id_dreammenutop, id_shop, new_window) 
				VALUES (null, '.(int)$params['new_id_shop'].', '.(int)$link['new_window'].')');
			
			$dreammenutop[$id]['new_id_dreammenutop'] = Db::getInstance()->Insert_ID();
		}
		...
		foreach($dreammenutop as $id => $link)
		{
			$lang = Db::getInstance()->executeS('
					...
					FROM '._DB_PREFIX_.'dreammenutop_lang 
					WHERE id_dreammenutop = '.(int)$link['id_dreammenutop'].' AND id_shop = '.(int)$params['old_id_shop']);
			
			foreach($lang as $l)
				Db::getInstance()->execute('
					INSERT IGNORE INTO '._DB_PREFIX_.'dreammenutop_lang (id_dreammenutop, id_lang, id_shop, label, link) 
					VALUES ('.(int)$link['new_id_dreammenutop'].', '.(int)$l['id_lang'].', '.(int)$params['new_id_shop'].', '.(int)$l['label'].', '.(int)$l['link'].' )');
		}
